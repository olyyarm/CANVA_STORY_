...
- в ноде «Таймлайн»: если сцен ещё нет, нажать «Подготовить главу» для разбивки, материала и автосборки сцен; после этого команда станет называться «Добрать недостающее» и сможет использовать `Ассеты → Comfy API · GPT Image 2 Low` для персонажей, общих локаций и недостающих фонов сцен;
