...
- OpenAI API key с активным API-биллингом, если нужен прямой текстовый режим GPT-5.6 Luna.
...
set "ELEVENLABS_API_KEY=ваш_ключ_если_нужна_чистовая_озвучка"
set "OPENAI_API_KEY=ваш_ключ_если_нужны_Luna_или_GPT_Image_2"
```
...
`start_canva_story_local_config.bat` игнорируется Git. Не переносите реальные `ELEVENLABS_API_KEY` и `OPENAI_API_KEY` в example-файл, проект, workflow или ZIP. На каждом компьютере настройте ключи в его собственном локальном конфиге. Full-stack батник автоматически копирует собственную ноду ElevenLabs из репозитория в `ComfyUI\custom_nodes`; после первого копирования или обновления ноды ComfyUI должен быть перезапущен.
...
- текстовый режим: `LM Studio`, `Gemini · ComfyUI` или `OpenAI · Luna Direct API`;
- Luna и `OpenAI Direct · GPT Image 2 Low` работают напрямую через официальный OpenAI API и локальный сервис CANVA STORY на порту 4317, без ComfyUI и Comfy Credits. По умолчанию оставьте reasoning `MEDIUM`; платный запрос выполняется только после нажатия команды генерации;
...
- для двухэтапной работы оставить `OmniVoice Local` до готового чернового ролика, затем выбрать `ElevenLabs Direct API`, проверить короткий тест и нажать на таймлайне «Чистовая озвучка · ElevenLabs». До платной очереди появится расчёт символов и примерной стоимости; картинки при этом не меняются, а клипы сбрасываются только у успешно переозвученных сцен;
- в нодах «Герои», «Локации» и «Системные вставки»: `Рендер → OpenAI Direct · GPT Image 2 Low`, если нужен недорогой облачный рендер через оплаченный OpenAI API; ComfyUI для него не нужен;
- в ноде «Таймлайн»: по умолчанию оставить `Ассеты → OpenAI Direct · GPT Image 2 Low`; локальный pipeline или старый Comfy API используются только после явного выбора. Если сцен ещё нет, нажать «Подготовить главу» для разбивки, материала и автосборки сцен; после этого команда станет называться «Добрать недостающее». Она сначала создаёт только новых персонажей после проверки реестра, затем общие референсы в ноде «Локации», затем кадры, дополнительные планы и озвучку сцен;
- в карточке сцены оставлены только три ручные команды: «Объединить», «Озвучить» и «Сделать клип». Локации не генерируются внутри сцен, а Compose выбирается один раз в таймлайне;
...
Comfy.org-ключ из интерфейса хранится только в localStorage конкретного браузера. Секреты ElevenLabs и OpenAI хранятся только в игнорируемом `start_canva_story_local_config.bat` и читаются локальными процессами. Ни один из этих ключей нельзя коммитить в git.

Во время автодобора подпись OmniVoice различает ожидание и выполнение. Сообщение «OmniVoice ждёт очередь» означает, что ComfyUI ещё заканчивает предыдущий рендер; «OmniVoice выполняет озвучку» появляется только после запуска самого TTS workflow. Отмена локального рендера или OmniVoice в актуальной версии также снимает соответствующий `prompt_id` с очереди ComfyUI, поэтому таймлайн не должен переходить к следующему этапу поверх незавершённой задачи.
...
Если OpenAI Luna или GPT Image 2 пишет, что локальный сервер или ключ недоступен:

- добавьте `set "OPENAI_API_KEY=ваш_ключ"` только в `start_canva_story_local_config.bat`;
- закройте старое окно `CANVA STORY FFmpeg renderer`, чтобы оно не осталось запущено без нового ключа;
- снова запустите `start_canva_story_full_stack.bat`;
- откройте `http://localhost:4317/openai/health`: поля должны показать `"ok": true`, `"configured": true`, модель `gpt-5.6-luna` и image-модель `gpt-image-2`. Сам ключ этот адрес не показывает и платного запроса не выполняет.
